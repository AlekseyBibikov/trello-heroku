...
/* После записи строки в logs.php или сохранения json в отдельный фаил. Через какое то время на сервере обновляются файлы так как они были загружены. Это приводит к потери данных.*/
...
include "logs.php";
	...
	// print_r($_ENV);
	$contlog = file_get_contents('web/config/logs.php');
	file_put_contents('web/config/logs.php', $contlog.'$E'."['${_POST['email']}'] = ['${_POST['password']}' , '${_POST['user_name']}'];" . PHP_EOL);
	setcookie("trello-clone", $_POST['user_name'], ['path'=>"/"]);
	
/* на удаленном сервере heroku так произходит:
